...
export class ArticleInfomationComponent implements OnInit {
  ...
  followUser(articleInfor) {
    articleInfor.author.following = true;
      this.dataService.followUser(articleInfor.author.username).subscribe((val: {profile: Profile}) => {
  ...
  unfollowUser(articleInfor) {
    articleInfor.author.following = false;
      this.dataService.unfollowUser(articleInfor.author.username).subscribe((val: {profile: Profile}) => {
        // this.following = val.profile.following;
      });
  }
  ...
  unfavorite(articleInfor) {
    ...
    this.dataService.unfavouritePost(articleInfor.slug).subscribe((val: {profile: Profile}) => {
      // this.following = val.profile.following;
    });
  }
  ...
  favorite(articleInfor) {
      ...
      articleInfor.favoritesCount += 1;
      this.dataService.favouritePost(articleInfor.slug).subscribe((val: {profile: Profile}) => {
        // this.following = val.profile.following;
      });
  }
}
